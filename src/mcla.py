...
def log_posterior(x, theta, eta, y, model, logprior, noise_cov, eta_sampler=None):
    ...
    eta_sampler: eta = sampler() -> (*, Nx, eta_dim) to use, overrides provided value of eta
    ...
    marg_like = np.mean(likelihood, axis=1)                 # (Ne, *)
    log_likelihood = np.sum(np.log(marg_like), axis=0)      # (*, )
...
def eig_mcla_pm(x_loc, theta_sampler, eta_sampler, model, prior_mean, prior_cov, N=100, M=100, Ne=10,
    ...
    def parallel_func(idx, y_i, kl_divergence):
        for curr_batch in range(num_batches):
            ...
            neg_logpost = lambda x, theta, eta: -log_posterior(x, theta, eta, y_curr, model, logprior, noise_cov,
                                                               eta_sampler=None)
